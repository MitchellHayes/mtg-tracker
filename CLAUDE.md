...
- `game_state.py` — In-memory state: `player_health` dict, `current_turn_id`, `monarch_id`, `initiative_id`, `day_night`, `turn_started_at`. Pydantic `Player` model. Single module-level SQLite connection (`_conn`). Call `_save()` after any mutation.
  - `update_player` auto-transfers Monarch/Initiative when a player is eliminated (life ≤ 0): to the active player, or to the next living player in turn order if the eliminated player was the active player.
  ...
  - `next_turn` advances in true turn (ID) order starting from `current_turn_id` via `_next_in_turn_order`, so a player eliminated on their own turn passes to the player after them (not the lowest living ID). Resets `speed_increased_this_turn = False` for all players.
