...
function PlayerController() {
  ...
  const prevLifeForCaptureRef = useRef(null)
  ...
  const prevTurnAudioRef = useRef(currentTurnId)
  useEffect(() => {
    const prev = prevTurnAudioRef.current
    prevTurnAudioRef.current = currentTurnId
  ...
  // When this player holds the Monarch/Initiative and loses life, prompt to pass the
  // token — combat damage to the holder steals it, but the backend can't tell combat
  // from other life loss, so surface it as a quick, dismissible reminder.
  useEffect(() => {
    if (!player) return
    const prev = prevLifeForCaptureRef.current
    prevLifeForCaptureRef.current = player.life
    if (prev === null || player.life >= prev) return
    if (isMonarch || hasInitiative) {
      setCapturePrompt(true)
      clearTimeout(captureTimerRef.current)
      captureTimerRef.current = setTimeout(() => setCapturePrompt(false), 8000)
    }
  }, [player?.life]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => () => { clearTimeout(captureTimerRef.current); clearTimeout(toastTimerRef.current) }, [])
  ...
  const passTokensTo = (opponentId) => {
    if (isMonarch) setMonarchApi(opponentId)
    if (hasInitiative) setInitiativeApi(opponentId)
    ...
    <div className={`pc-root ${isEliminated ? 'eliminated' : ''}`}>
            ...
            <div className='pc-extras'>
              ...
              <div className='pc-module'>
                ...
                <button
                  ...
                  disabled={isEliminated}
                  onClick={() => {
                    if (!hasInitiative) showToast('You took the Initiative — venture into the Undercity now.')
                    setInitiativeApi(hasInitiative ? null : playerId)
                    vibrate(40)
                  }}
                >
      ...
      <div className='pc-toolbar'>
        ...
        <div className='pc-toolbar-center'>
          {isMyTurn ? (
            // Shown even when eliminated: an active player who dies still needs to pass
            // their turn so play can continue.
      ...
      {capturePrompt && (isMonarch || hasInitiative) && opponents.length > 0 && (
        <div className='pc-capture-prompt'>
          <button className='pc-capture-dismiss' onClick={() => setCapturePrompt(false)} aria-label='Dismiss'>✕</button>
          <div className='pc-capture-text'>
            Combat damage from an opponent? Pass {isMonarch && hasInitiative ? 'the crown & initiative' : isMonarch ? 'the crown' : 'the initiative'} to:
          </div>
          <div className='pc-capture-opps'>
            {opponents.map((opp) => (
              <button key={opp.id} className='pc-capture-opp' onClick={() => passTokensTo(opp.id)}>{opp.name}</button>
            ))}
          </div>
        </div>
      )}
